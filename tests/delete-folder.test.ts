...
test('deleteFolder moves notes up one level, preserving subfolders', async () => {
  ...
  try {
    folders.createFolder('Projects/Work/Deep');
    ...
    assert.deepEqual(folders.folderTree(), ['Projects', 'Projects/Work', 'Projects/Work/Deep']);
    const parent = organization.deleteFolder('Projects/Work');
    ...
    assert.deepEqual(folders.folderTree(), ['Projects', 'Projects/Deep']);
