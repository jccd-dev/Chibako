...
import { authenticateApiKey, hasAnyScope } from "../server/auth/api-key-authorization";

// ---------- auth ----------

function authorized(required: readonly string[]): boolean {
  const configured = process.env.CHIBAKO_API_KEY;
  if (!configured) return true; // direct DB access, no key configured
  const principal = authenticateApiKey(configured);
  return principal !== null && hasAnyScope(principal.scopes, required);
}
